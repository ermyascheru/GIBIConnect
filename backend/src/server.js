...
const { pool, ensureAdminDashboardTables } = require('./config/database');

const PORT = process.env.PORT || 5000;

const startServer = async () => {
  const tablesReady = await ensureAdminDashboardTables();
  ...
  const server = app.listen(PORT, async () => {
    ...
    try {
      ...
      if (!tablesReady) {
        console.warn('⚠️ Admin dashboard schema check completed with warnings.');
      }
    } catch (err) {
