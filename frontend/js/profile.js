import { getUser, getUserSavedItems, removeSavedInstitution, removeSavedProgram, removeSavedResource, getInstitutionLogoUrl, getInstitutionInitials } from './api.js';
  ...
  if (activeBookmarkTab === 'institutions') {
    body.innerHTML = `
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ${items.map(i => {
          const logoSrc = getInstitutionLogoUrl(i);
          const initials = getInstitutionInitials(i.name);
          ...
          <div class="p-4 rounded-2xl border border-outline-variant/15 bg-surface-container-low flex items-center justify-between group">
            <div onclick="window.location.href='institution.html?id=${i.id}'" class="flex items-center gap-3 cursor-pointer flex-grow overflow-hidden">
              <div class="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 border border-slate-700 overflow-hidden shadow-sm flex-shrink-0 p-1 flex items-center justify-center relative">
                ${logoSrc ? `
                  ...
                  <div style="display: none;" class="w-full h-full rounded bg-emerald-500/10 text-[#10B981] font-bold text-xs items-center justify-center">
                    ${initials}
                  </div>
                ` : `
                  <div class="w-full h-full rounded bg-emerald-500/10 text-[#10B981] font-bold text-xs flex items-center justify-center">
                    ${initials}
                  </div>
                `}
              </div>
