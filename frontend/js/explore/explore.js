import { getInstitutions, getPrograms, getInstitutionLogoUrl, getInstitutionInitials } from '../core/api.js';
...
async function initExplore() {
  ...
  try {
    ...
    if (instGrid) {
      const institutions = (instRes && instRes.success && Array.isArray(instRes.data) && instRes.data.length > 0) ? instRes.data : [];
      if (institutions.length > 0) {
        instGrid.innerHTML = institutions.map(inst => {
            ...
            <div>
              <div class="relative h-32 -mx-4 -mt-4 mb-3 rounded-t-2xl overflow-hidden bg-slate-800">
                ...
                <div class="absolute bottom-2.5 left-3 flex items-center gap-2">
                  <div class="w-9 h-9 rounded-lg bg-white dark:bg-slate-900 border border-slate-700 overflow-hidden shadow-sm flex-shrink-0 p-1 flex items-center justify-center relative">
                    ${logoSrc ? `
                      <img src="${logoSrc}" alt="Logo" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" class="w-full h-full object-contain">
                      <div style="display: none;" class="w-full h-full rounded bg-emerald-500/10 text-[#10B981] font-bold text-[10px] items-center justify-center">
                        ${initials}
                      </div>
                    ` : `
                      <div class="w-full h-full rounded bg-emerald-500/10 text-[#10B981] font-bold text-[10px] flex items-center justify-center">
                        ${initials}
                      </div>
                    `}
                  </div>
        ...
        instGrid.innerHTML = '<div class="col-span-full text-center py-6 text-xs text-on-surface-variant">No universities found.</div>';
      }
    }

    if (progGrid) {
      const programs = (progRes && progRes.success && Array.isArray(progRes.data) && progRes.data.length > 0) ? progRes.data : [];
      if (programs.length > 0) {
        ...
        progGrid.innerHTML = '<div class="col-span-full text-center py-6 text-xs text-on-surface-variant">No programs found.</div>';
      }
    }
  } catch (err) {
    console.error('Error loading explore data:', err);
    if (instGrid) instGrid.innerHTML = '<div class="col-span-full text-center py-6 text-xs text-rose-500">Failed to load institutions. Please refresh.</div>';
    if (progGrid) progGrid.innerHTML = '<div class="col-span-full text-center py-6 text-xs text-rose-500">Failed to load programs. Please refresh.</div>';
  }
}
